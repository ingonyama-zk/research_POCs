...
for query_proof in query_proofs.iter().take(num_rounds.try_into().unwrap()) {
    ...
    challenge_vec.push(<Transcript as TranscriptProtocol<F>>::challenge_scalar(transcript, b"challenge"));
}
...
debug!("nonce {:?}",nonce);
let out: Vec<u8> = hash_fuse(current_challenge.to_bytes_le(), nonce.to_le_bytes().to_vec());
let out_lead_zeros: usize = num_leading_zeros(out);
assert_eq!(out_lead_zeros,fri_config.pow_bits,"Nonce does not satisfy POW condition");
...
for q in 0..fri_config.num_queries{
    ...
    for r in 0..nr{      
        ...
        let rou_inv = rou.inv();
            ...
            let l_odd = (leafs_q[r]-leafs_sym_q[r])*two_inv* (pow(rou_inv,index_q[r].try_into().unwrap()));
           ...
           debug!("queryno:{:?}, round: {:?},Index: {:?}, leaf: {:?}, Index_sym: {:?}  ,leaf_sym: {:?}, computed_next_leaf: {:?}",q,r,index_q[r],leafs_q[r],index_symq[r], leafs_sym_q[r],l_next);
            if r == nr-1{
                assert_eq!(l_next,friproof.final_poly[index_q[r] % fri_config.stopping_size],"collinearity test failed at final poly");
            }
            else{
                assert_eq!(l_next,leafs_q[r+1],"Collinearity test failed at round {:?}",r);
            }
    }
    rq+=nr;
}
