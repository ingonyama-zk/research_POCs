...
use icicle_core::{field::Field, traits::FieldImpl};
...
impl KeccakTranscript {
    ...
    fn hasher(&self) -> Keccak256 {
        // let mut packed = [0_u8; 32].to_vec();
        let mut data = self.state.to_vec();
        data.append(self.n_rounds.to_le_bytes().to_vec().as_mut());
        // packed.append(&mut self.n_rounds.to_le_bytes().to_vec());
        // Note we add the extra memory here to improve the ease of eth integrations
        Keccak256::new().chain_update(data)
            // .chain_update(self.state)
            // .chain_update(&self.n_rounds.to_le_bytes().to_vec())
    }
...
impl Transcript for KeccakTranscript {
    fn new<F:FieldImpl>(label: &'static [u8], num_vars:u32, degree:u32, claimed_sum: F, seedrng: F, mut entry0: Vec<u8>) -> Self {
        // Hash in the label
//        assert!(label.len() < 33);
        let mut data = label.to_vec();
        data.append(&mut num_vars.to_le_bytes().to_vec());
        data.append(&mut degree.to_le_bytes().to_vec());
        data.append(&mut claimed_sum.to_bytes_le().to_vec());
        data.append(&mut seedrng.to_bytes_le().to_vec());
        data.append(&mut entry0);
        let hasher = Keccak256::new().chain_update(data);
    ...
    fn append_scalar<F: FieldImpl>(&mut self, scalar: &F) {
        // let mut buf = vec![];
        let mut buf = scalar.to_bytes_le();
        // scalar.to_bytes_le(&mut buf).unwrap();
    ...
    fn append_scalars<F: FieldImpl>(&mut self, scalars: &[impl Borrow<F>]) {
        self.append_message(b"begin_append_vector");
        self.append_bytes(scalars.len().to_le_bytes().as_ref());
    ...
    fn challenge_scalar<F: FieldImpl>(&mut self) -> F {
        let mut buf = vec![0u8; F::one().to_bytes_le().len()];
        buf = self.hasher().finalize().to_vec();
...
pub trait Transcript: Clone + Sync + Send + 'static {
    fn new<F: FieldImpl>(label: &'static [u8], num_vars: u32, degree: u32, claimed_sum: F, seedrng: F, entry0: Vec<u8>) -> Self;
    #[cfg(test)]
    fn compare_to(&mut self, other: Self);
    fn append_message(&mut self, msg: &'static [u8]);
    fn append_bytes(&mut self, bytes: &[u8]);
