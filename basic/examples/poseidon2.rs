...
pub fn main() {
    ...
    let poseidon_state_size = 2; //each leaf is 2 elements, each 4 bytes
    let leaf_size: u64 = 8; //
    let nof_leaves = 1 << 20 as usize;
    let nof_elements = nof_leaves * poseidon_state_size;


    let mut test_vec =
        vec![Frm31::from_u32(random::<u32>()); nof_elements];
    println!(
    ...
    let hasher: Hasher =
        Poseidon2::new::<Frm31>(poseidon_state_size.try_into().unwrap(), None).unwrap();
    let compress: Hasher =
        Poseidon2::new::<Frm31>(poseidon_state_size.try_into().unwrap(), None).unwrap();
    //tree config
    let tree_config = MerkleTreeConfig::default();
    let merk_tree = compute_binary_tree(
        test_vec.clone(),
        leaf_size,
        hasher,
        compress,
    ...
    println!(
        ...
        merk_tree.get_root::<Frm31>().unwrap()
    );
    ...
    let merkle_proof = merk_tree
        .get_proof::<Frm31>(
            test_vec_slice,
            random_test_index.try_into().unwrap(),
            false,
            &tree_config,
        )
        .unwrap();
