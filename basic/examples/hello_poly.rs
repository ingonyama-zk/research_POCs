...
(P::Field,P::Field,P::Field,P::Field){
    ...
    (t1_l.eval(&alpha),t1_r.eval(&alpha),t2_l.eval(&alpha),t2_r.eval(&alpha))
}
...
println!("(p_1(x)+p_2(x))^2+(p_1(x)-p_2(x))^2 eval at x: {:?}, from generic {:?}", t1_l_one,a1);
println!("2 (p_1(x)^2+p_2(x)^2 eval at x: {:?}, from generic {:?}",t1_r_one,a2);
assert_eq!(t2_l_one,t2_r_one);
assert_eq!(a3,a4); 
assert_eq!(t2_l_one,a3); 

println!("(p_1(x)+p_2(x))^2-(p_1(x)-p_2(x))^2 eval at x: {:?}, from generic {:?}",t2_l_one,a3);
println!("4 p_1(x)*p_2(x) eval at x: is {:?},from generic {:?}",t2_r_one,a4);
}